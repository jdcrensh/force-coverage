...
async.auto
  ...
  pkgTestsDestruct: ['pkgDelete', (done) ->
    ...
    xml.writePackage ApexClass: ['CoverageInflation'], dest, done
  ]
  ...
  inflationClass: ['parseCoverage', (done, res) ->
    ...
    async.waterfall [
      (done) ->
        fp = path.join 'pkg', 'classes', 'CoverageInflation.cls-meta.xml'
      ...
      (res, done) ->
        fp = path.join 'pkg', 'classes', 'CoverageInflation.cls'
      ...
      (done) ->
        ...
        {version} = argv
        components = ApexClass: ['CoverageInflation']
